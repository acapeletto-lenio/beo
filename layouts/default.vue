    ...
    <header>
      <div class="nav-logo" @click="handleHome()">
        ...
        <div class="meganavsection descripted">
          <p>
            95 indicadores develando de dónde viene y hacia dónde va la
      ...
      <div class="top-actions">
        <div>
          ...
          <input
            ...
            ref="masterSearcher"
          />
          <button class="searchsubmit" @input="handleSearch($event)">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="48"
              viewBox="0 -960 960 960"
        ...
        <div
          class="view-mode"
          v-if="$route.path === '/' || $route.path.includes('/search')"
        >
          <button
            v-for="(mode, key) in viewModes"
            :class="{ active: key === $state.activeMode }"
            @click="setMode(key)"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
  ...
  data() {
    return {
      ...
      viewModes: {
        ...
        table:
          "M820-121H180q-24 0-42-18t-18-42v-599q0-24 18-42t42-18h640q24 0 42 18t18 42v599q0 24-18 42t-42 18ZM180-639h640v-141H180v141Zm150 60H180v398h150v-398Zm340 0v398h150v-398H670Zm-60 0H390v398h220v-398Z",
      },
    };
  },
    ...
    handleSearch(event) {
      ...
      if (searchTerm.length > 2) {
        ...
        for (let key in this.$state.savedIndex) {
          if (
            Object.values(this.$state.savedIndex[key]).some((val) => {
              if (val != "" && typeof val === "string") {
                return this.removeAccents(val.toLowerCase()).includes(
                  searchTerm
                );
              }
              return false;
        ...
        if (Object.keys(results).length === 1) {
          var searchTermParent = this.findRootParent(
            Object.entries(results)[0][0]
          );
          this.$router.push({
            path: `/${searchTermParent}/${Object.entries(results)[0][0]}`,
          });
          //this.$state.isSearching = false
        } else {
