  ...
  <section class="lastup">
      ...
      <div class="scorecard">
        <div style="align-items: baseline">
          <div style="justify-content: center; padding-top: 0">
            <div style="color: #676c6f; padding-top: 0">Último Dato</div>
          </div>
        ...
        <div class="sliceheatmap">
          <div
            v-for="slice in 12"
            v-if="kpi.frec === 'Mensual'"
            :style="{ background: getVariationHeatmap(slice) }"
          ></div>
        </div>
      </div>
    </div>
    <div>
      <div class="scorecard highlight">
        <div
          class="single-legend"
          v-for="(kpi, parent) in kpi.dimensions"
          :key="`${kpi.label}`"
        >
          <div>
          ...
          <div>
            <div>
              {{ kpi.sym }}{{ kpi.data[kpi.data.length - 1]?.y.toFixed(2)
              }}{{ kpi.unit }}
            </div>
            <div
              class="deltacontainer"
              :class="{ negative: getVariation(kpi) < 0 }"
            >
              <i v-if="getVariation(kpi) > 0">▲</i>
              <i v-if="getVariation(kpi) < 0">▼</i>
              <span class="delta"
                >{{ getVariation(kpi).replace("-", "") }}%</span
              >
            </div>
          </div>
        </div>
    ...
    <div v-if="metadata !== false">
      ...
      <div class="scorecard">
        <div>
          <div>Último Dato:</div>
          <div>
            {{ kpi.dimensions[0].data[kpi.dimensions[0].data.length - 1].x }}
          </div>
        </div>
        <div>
          <div>Frecuencia:</div>
        ...
        <div>
          <div>Fuente:</div>
          <div>
            <a target="_blank" :href="kpi.fur">{{ kpi.fu }}</a>
          </div>
        </div>
        <div>
          <div>Método:</div>
          <div>
            <a target="_blank" :href="kpi.fdr">{{ kpi.fd }}</a>
          </div>
        </div>
        <div>
          <div>Descargar:</div>
          <div>
            <a :href="kpi.data">CSV</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a
              :href="kpi.data"
              >JSON</a
            >&nbsp;&nbsp;|&nbsp;&nbsp;<a :href="kpi.data">XLS</a>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    processedDate() {
      var pepe = new Date(
        this.kpi.dimensions[0].data[this.kpi.dimensions[0].data.length - 1].x
      )
      ...
      if (this.kpi.frec === "Mensual") {
        const lastDate = new Date(
          this.kpi.dimensions[0].data.slice(-1)[0].x + "T00:00:00"
        );
    ...
    getLastValue() {
      var lastValue =
        this.kpi.dimensions[0].data[
          this.kpi.dimensions[0].data.length - 1
        ]?.y.toFixed(2);
      return lastValue;
    },
    getVariation(legend) {
