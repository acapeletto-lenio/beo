...
export default {
  ...
  methods: {
    ...
    processedDate() {
      const lastData = this.kpi[0].data[this.kpi[0].data.length - 1];
      const date = new Date(lastData.x);

      if (this.staticKpi.frec === "Mensual") {
        // Ensure it's the last day of the month
        date.setMonth(date.getMonth() + 1);
        date.setDate(0);
        return date
          .toLocaleString("es", { month: "long", year: "numeric" })
          .replaceAll("de", "");
      } else if (this.staticKpi.frec === "Anual") {
        return `Año ${date
    ...
    generateChart() {
      ...
      const scaleX = d3
        ...
        .nice(timeIntervals[this.zoomLevel].interval);
