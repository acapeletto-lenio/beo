<template>
  <div class="pepecontainer" :class="{ index }">
      ...
      <div class="innerflexer" v-if="index === undefined">
        ...
        <div class="subinnerflexer">
          <div
            ...
            v-if="
              staticKpi.frec === 'Diaria' ||
              (staticKpi.frec === 'Mensual' && item !== 'Diaria') ||
              (staticKpi.frec === 'Anual' && item === 'Anual')
            "
      ...
      <div class="chartcontainer" ref="c">
        <template v-if="defaultView">
          ...
          <div
            ...
            :class="{ dragging }"
            :style="{
              'grid-template-columns': `${axisBottom
                .map((c) => `${c.width}px`)
                .join(' ')} 1fr`,
              cursor: recursor,
            }"
          >
            <div
              v-for="i in axisBottom.length + 1"
              :key="`${i}-col`"
          ...
          >
            <clipPath id="clip">
              <rect
                :x="`${axisBottom[0].width}px`"
                y="0"
                :height="chartHeight"
                :width="chartWidth - 50 - axisBottom[0].width"
              ></rect>
            </clipPath>
            ...
            <g class="axis xAxis">
              <g
                v-for="tick in axisBottom"
                :transform="`translate(${tick.left},${chartHeight - 20})`"
                :data-date="tick.value"
              >
            ...
            <g
              class="axis yAxis"
              :class="{
                even: axisRight.length % 2 === 0,
                odd: axisRight.length % 2 !== 0,
              }"
            >
              <g
                v-for="tick in axisRight"
                :transform="`translate(${chartWidth - 30},${tick.top})`"
              >
    ...
    <!-- Index Legends -->
    <div class="legends-container" v-if="index">
      <!--       <i v-if="index">&#x23F1; {{ processedDate() }}</i> 
 -->
      <div class="legends" style="min-height: 14px">
        <div
          class="single-legend"
          v-for="(kpi, parent) in kpi.slice(0, 5)"
          :key="`${kpi.label}`"
        >
          <span class="circle" :style="{ background: kpi.color }"></span>
          <span>{{ kpi.label }}</span>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      staticKpi: JSON.parse(
        JSON.stringify(require(`~/static/data/${this.data}.json`))
      ),
      startX: "",
      dragDates: [],
      axisBottom: [],
  ...
  computed: {
    startDates() {
      ...
      const subtractDate = (years, months) => (
  ...
  methods: {
    getCols(i) {
      return (
        Math.ceil(i % (this.axisBottom.length + 1)) ||
        this.axisBottom.length + 1
      );
    },
    getColDate(i) {
      return (
        this.axisBottom[i]?.date ??
        this.kpi[0].data[this.kpi[0].data.length - 1].x
      );
    },
    processedDate() {
      const date = new Date(this.kpi[0].data[this.kpi[0].data.length - 1].x);
      return this.staticKpi.frec === "Mensual"
        ? date
            .toLocaleString("es", { month: "long", year: "numeric" })
            .replaceAll("de", "")
        : this.staticKpi.frec === "Anual"
        ? `Año ${date
            .toLocaleDateString("es", { year: "numeric" })
            .replaceAll("de", "")}`
        : date
    ...
    refreshData() {
      ...
      const filter = (data, start, end) =>
        data.filter(
          (item) => (!start || item.x >= start) && (!end || item.x <= end)
        );

      //Aggregate by daily, monthly and yearly
      const aggregate = (data, period, fruc) =>
        Object.entries(
          data.reduce((groups, { x, y }) => {
      ...
      this.kpi = Object.values(this.staticKpi.dimensions).map(
        ({ label, color, data }) => {
          let filteredData = start || end ? filter(data, start, end) : data;
          if (agg) {
            const period =
              agg.toLowerCase() === "anual"
                ? 4
                : agg.toLowerCase() === "mensual"
                ? 7
                : this.staticKpi.frec === "Diaria"
                ? 10
                : 7;
    ...
    generateChart() {
      ...
      const scaleX = d3
        .scaleTime()
        .domain([
          parseTime(this.kpi[0].data[0].x),
          parseTime(this.kpi[0].data[this.kpi[0].data.length - 1].x),
        ])
        .range([0, this.$refs.c.clientWidth - 50])
        .nice(timeIntervals[0].interval);

      //create the x axis object with value, translate and cell width
      this.axisBottom = scaleX.ticks().flatMap((d, i, ticks) => [
        {
          value: d3.timeFormat(timeIntervals[0].format)(d),
      ...
      const [minValue, maxValue] = [
        this.kpi.min ?? d3.min(valuesData) * 0.9,
        this.kpi.max ?? d3.max(valuesData) * 1.05,
      ];

      const scaleY = d3
        .scaleLinear()
        .domain([minValue, maxValue])
        .range([this.$refs.c.clientHeight - 30, 10])
        .nice();

      //create the y axis object with value and translate
      this.axisRight = scaleY
        .ticks()
        .map((d) => ({ value: d, top: scaleY(d) }));

      // create a path generator based on chart type and add it to each dimension
      let pathGenerator = d3
        .line()
        .x((d) => scaleX(parseTime(d.x)))
        .y((d) => scaleY(d.y))
        .defined((d) => d.y !== null);
    ...
    startDrag(e) {
      ...
      console.log("clientX:", e.clientX); // Log the clientX value
      console.log("colStart:", e.target.dataset.colStart); // Log the colStart value
      console.log("colEnd:", e.target.dataset.colEnd); // Log the colEnd value
    ...
    hoverling(e) {
      if (this.dragging) {
        ...
        this.$refs.reselecter.style.gridColumnStart =
          xDir > 0
            ? e.target.dataset.colStart
            : this.$refs.reselecter.style.gridColumnStart;
        this.$refs.reselecter.style.gridColumnEnd =
          xDir < 0
...
.ranger {
  ...
  > div {
    ...
    &.reselecter-cell {
      ...
      border-right: 2px solid rgba(255, 255, 0, 0.25);
      display: none;
      pointer-events: none;
    }
  }
