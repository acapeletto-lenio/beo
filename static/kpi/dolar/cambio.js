module.exports = (async function () {
  ...
  const reambito = {
    oficial:
      "https://mercados.ambito.com//dolar/oficial/historico-general/01-01-1900/01-01-2100",
    ...
    ahorro:
      "https://mercados.ambito.com//dolarahorro/historico-general/01-01-1900/01-01-2100",
  };
  ...
  for (let [key, value] of Object.entries(reambito)) {
    ...
    for (let i = 1; i < Math.max(...fillLength); i++) {
      payload[key][i] = { x: 0, y: null };
    }
    for (let i = 1; i < data.length; i++) {
      payload[key][i].x = new Date(data[i][0].split("/").reverse().join("-"))
        .toISOString()
        .substring(0, 10);
      if (key === "oficial") {
