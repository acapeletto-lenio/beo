module.exports = (async function () {
  ...
  const url =
    "https://infra.datos.gob.ar/catalog/sspm/dataset/131/distribution/131.1/download/principales-tasas-interes-referencia-principales-economias-mundo-frecuencia-mensual.csv";
  const fed = await parsers.datosGobarCSV(0, 2, url);
  const euro = await parsers.datosGobarCSV(0, 1, url);
  const brasil = await parsers.datosGobarCSV(0, 3, url);
  ...
  for (let [key, value] of Object.entries(reambito)) {
    ...
    for (let i = 0; i < Math.max(...fillLength); i++) {
      payload[key][i] = { x: 0, y: 0 };
    }
  ...
  const post = {
    ...
    frec: parsers.detectDataType(fed),
    fruc: parsers.detectAggregationFunction(fed),
