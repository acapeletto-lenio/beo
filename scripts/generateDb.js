...
async function processVariation(src) {
  ...
  for (const singleFolder of folders) {
    if (singleFolder.slice(-3) === ".js") {
      try {
        var post = await require(`../static/data/${singleFolder
          .split("/")
          .pop()
          .slice(0, -3)}.json`);
...
async function processNamers() {
  ...
  for (let e = 0; e < files.length; e++) {
    kpeis[await require(`../static/data/${files[e].slice(0, -3)}.json`).kpi] =
      await require(`../static/data/${files[e].slice(0, -3)}.json`).t;
  }
...
async function processTable() {
  ...
  files = [...new Set(files.flat(1))];
  for (let e = 0; e < files.length; e++) {
    var singleKpi = await require(`../static/data/${files[e].slice(
      0,
      -3
    )}.json`);
    kpeis[singleKpi.kpi] = {
  ...
  writeFileSyncRecursive(
    `./static/kpisearchable.json`,
    JSON.stringify(searchkpeis)
  );
...
async function processFolders() {
  ...
  writeFileSyncRecursive(
    `./static/refolders.json`,
    JSON.stringify(orderedtree)
  );
...
async function magicDistribution() {
  ...
  writeFileSyncRecursive(
    `./static/megapepe.json`,
    JSON.stringify(transformedPepe)
  );
}
...
async function processItems(arr) {
  ...
  const progressBar = new ProgressBar(
    `\x1b[1m☺ Fetching data (:current/:total) \x1b[0m \x1b[32m|\x1b[0m:bar\x1b[32m|\x1b[0m :percent :file`,
    {
  ...
  for (let i = 0; i < arr.length; i += limit) {
    await Promise.all(
      arr.slice(i, i + limit).map(async (kpi) => {
        try {
...
processItems(
  glob.sync("**", { cwd: `static/kpi/` }).filter((kpi) => kpi.endsWith(".js"))
);
